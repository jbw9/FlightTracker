...
// Your actual flight details - JL55 and JL729
export const FLIGHT_CONFIG: FlightConfig[] = [
  {
    id: "jl55-ord-nrt",
    flightNumber: "JL55",
    callsign: "JAL55",
    // icao24: "JA123A", // Add if you find the specific aircraft ICAO code
    from: {
      code: "ORD",
      city: "Chicago",
    ...
    departure: "2025-06-25T11:50:00-05:00",  // Updated to 2025 (tomorrow) - Chicago time (CDT)
    arrival: "2025-06-26T15:35:00+09:00",    // Updated to 2025 - Tokyo time
    aircraft: "Boeing 787-9",  // JL55 typically uses 787-9
    airline: "Japan Airlines",
    priority: "high",  // High priority for tracking
  ...
  },
  {
    ...
    flightNumber: "JL729",
    callsign: "JAL729",
    // icao24: "JA456B", // Add if you find the specific aircraft ICAO code
    from: {
      code: "NRT",
      city: "Tokyo Narita",
      coordinates: [35.7647, 140.3864]
    ...
    departure: "2025-06-27T10:50:00+09:00",  // Updated to 2025 - Tokyo time
    arrival: "2025-06-27T16:55:00+07:00",    // Updated to 2025 - Jakarta time
    aircraft: "Boeing 787-8",  // JL729 typically uses 787-8
    airline: "Japan Airlines",
    priority: "high",  // High priority for tracking
    trackingEnabled: true
  }
...
export const AIRPORTS = {
  LAX: { code: "LAX", city: "Los Angeles", coordinates: [34.0522, -118.2437] as [number, number] },
  JFK: { code: "JFK", city: "New York", coordinates: [40.6413, -73.7781] as [number, number] },
  LHR: { code: "LHR", city: "London", coordinates: [51.4700, -0.4543] as [number, number] },
  NRT: { code: "NRT", city: "Tokyo Narita", coordinates: [35.7647, 140.3864] as [number, number] },
  CDG: { code: "CDG", city: "Paris", coordinates: [49.0097, 2.5479] as [number, number] },
  FRA: { code: "FRA", city: "Frankfurt", coordinates: [50.0379, 8.5622] as [number, number] },
  DXB: { code: "DXB", city: "Dubai", coordinates: [25.2532, 55.3657] as [number, number] },
  SIN: { code: "SIN", city: "Singapore", coordinates: [1.3644, 103.9915] as [number, number] },
  SYD: { code: "SYD", city: "Sydney", coordinates: [-33.9399, 151.1753] as [number, number] },
  ORD: { code: "ORD", city: "Chicago O'Hare", coordinates: [41.9742, -87.9073] as [number, number] },
  CGK: { code: "CGK", city: "Jakarta", coordinates: [-6.1256, 106.6558] as [number, number] },
};
...
export const getFlightsByPriority = (priority: 'high' | 'medium' | 'low'): FlightConfig[] => {
  return FLIGHT_CONFIG.filter(flight => flight.priority === priority && flight.trackingEnabled);
};

export const getUpdateInterval = (priority: 'high' | 'medium' | 'low'): number => {
  switch (priority) {
    case 'high': return TRACKING_CONFIG.HIGH_PRIORITY_INTERVAL;
    case 'medium': return TRACKING_CONFIG.MEDIUM_PRIORITY_INTERVAL;
    case 'low': return TRACKING_CONFIG.LOW_PRIORITY_INTERVAL;
    default: return TRACKING_CONFIG.MEDIUM_PRIORITY_INTERVAL;
  }
};
